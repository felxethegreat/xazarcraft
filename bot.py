...
async def yardim(ctx):
    embed = discord.Embed(title="📋 NexList Bot Komutları", color=0x5865F2)
    ...
    embed.add_field(name="👤 Kullanıcı", value="`e!profil` `e!sunucu` `e!pfp @kullanici` `e!bilgi`", inline=False)
    embed.add_field(name="📊 İstatistik", value="`e!sestop` `e!sestop-sifirla` `e!mesajtop` `e!mesajtop-sifirla`", inline=False)
    embed.add_field(name="🔨 Ban", value="`e!banaffi` `e!banlist`", inline=False)
    ...
    embed.set_footer(text="NexList")
    await ctx.send(embed=embed)

@bot.command(name="bilgi")
async def bilgi(ctx):
    embed = discord.Embed(title="🤖 NexList Bot Hakkında", color=0x5865F2)
    embed.add_field(name="👑 Geliştirici", value="felxethegreat", inline=True)
    embed.add_field(name="📋 Prefix", value="`e!` veya `/`", inline=True)
    embed.add_field(name="🌐 Sunucu", value="NexList", inline=True)
...
async def pfp(ctx, uye: discord.Member = None):
    ...
    embed.set_footer(text=f"{uye.name} — NexList")
    msg = await ctx.send(embed=embed)
    await asyncio.sleep(0.5)
    try:
        await ctx.message.delete()
...
async def sestop(ctx):
    ...
    embed.set_footer(text="NexList | e!sestop-sifirla ile sıfırla")
    await ctx.send(embed=embed)
...
async def mesajtop(ctx):
    ...
    embed.add_field(name="Sıralama", value="\n".join(satirlar), inline=False)
    embed.set_footer(text="NexList | e!mesajtop-sifirla ile sıfırla")
    await ctx.send(embed=embed)
...
async def slash_embed(interaction: discord.Interaction, baslik: str, icerik: str, renk: str = "5865F2"):
    ...
    embed.set_footer(text=f"NexList | {interaction.user.name}")
    await interaction.response.send_message("✅ Gönderildi.", ephemeral=True)
...
async def duyuru(interaction: discord.Interaction, kanal: discord.TextChannel, mesaj: str):
    ...
    embed.set_footer(text=f"Duyuran: {interaction.user.name} | NexList")
    await kanal.send(embed=embed)
...
async def otorol_mesaj_ayarla(interaction: discord.Interaction, mesaj: str, emoji: str, rol: discord.Role):
    ...
    embed.set_footer(text=f"NexList | {emoji} emojisine tıkla → {rol.name} rolünü al")
