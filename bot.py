...
# KairoHub | Minecraft Hile Sunucusu | Kurucular: dwoq0 & felxe
...
bot = commands.Bot(command_prefix=commands.when_mentioned_or("k!", "K!", "/"), intents=intents, help_command=None)
...
async def yardim(ctx):
    embed = discord.Embed(title="📋 KairoHub Bot Komutları", color=0x5865F2)
    embed.add_field(name="🛡️ Moderasyon", value="`k!ban` `k!kick` `k!mute` `k!unban` `k!kilit` `k!kilitac` `k!sil` `k!uyar` `k!uyarılar` `k!uyarısıfırla`", inline=False)
    embed.add_field(name="👤 Kullanıcı", value="`k!profil` `k!sunucu` `k!pfp @kullanici` `k!bilgi`", inline=False)
    embed.add_field(name="📊 İstatistik", value="`k!sestop` `k!sestop-sifirla` `k!mesajtop` `k!mesajtop-sifirla`", inline=False)
    embed.add_field(name="🔨 Ban", value="`k!banaffi` `k!banlist`", inline=False)
    ...
    embed.set_footer(text="KairoHub")
    await ctx.send(embed=embed)

@bot.command(name="bilgi")
async def bilgi(ctx):
    embed = discord.Embed(title="🤖 KairoHub Bot Hakkında", color=0x5865F2)
    embed.add_field(name="👑 Geliştirici", value="dwoq0 & felxe", inline=True)
    embed.add_field(name="📋 Prefix", value="`k!` veya `/`", inline=True)
    embed.add_field(name="🌐 Sunucu", value="KairoHub", inline=True)
    embed.add_field(name="⚙️ Özellikler", value="Minecraft hile sunucusu KairoHub için moderasyon, filtreler, çekiliş, ticket, otorol ve daha fazlası!", inline=False)
    embed.set_footer(text="KairoHub")
    await ctx.send(embed=embed)
...
async def pfp(ctx, uye: discord.Member = None):
    ...
    embed.set_footer(text=f"{uye.name} — KairoHub")
    msg = await ctx.send(embed=embed)
    await asyncio.sleep(0.5)
    try:
        await ctx.message.delete()
...
async def sestop(ctx):
    ...
    embed.set_footer(text="KairoHub | k!sestop-sifirla ile sıfırla")
    await ctx.send(embed=embed)
...
async def mesajtop(ctx):
    ...
    embed.add_field(name="Sıralama", value="\n".join(satirlar), inline=False)
    embed.set_footer(text="KairoHub | k!mesajtop-sifirla ile sıfırla")
    await ctx.send(embed=embed)
...
async def slash_embed(interaction: discord.Interaction, baslik: str, icerik: str, renk: str = "5865F2"):
    ...
    embed.set_footer(text=f"KairoHub | {interaction.user.name}")
    await interaction.response.send_message("✅ Gönderildi.", ephemeral=True)
...
async def duyuru(interaction: discord.Interaction, kanal: discord.TextChannel, mesaj: str):
    ...
    embed.set_footer(text=f"Duyuran: {interaction.user.name} | KairoHub")
    await kanal.send(embed=embed)
...
async def otorol_mesaj_ayarla(interaction: discord.Interaction, mesaj: str, emoji: str, rol: discord.Role):
    ...
    embed.set_footer(text=f"KairoHub | {emoji} emojisine tıkla → {rol.name} rolünü al")
