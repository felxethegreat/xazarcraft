...
def reklam_var_mi(metin: str, ekstra_liste: list) -> bool:
    metin_lower = metin.lower()
    # Varsayılan Discord davet/nitro tespiti
    for pattern in VARSAYILAN_REKLAM:
        if re.search(pattern, metin_lower):
            return True
    # URL tespiti (http/https)
    if re.search(r"https?://\S+", metin_lower):
        return True
    # Kullanıcının eklediği ekstra kelimeler/linkler
...
def sure_parse(sure_str: str) -> int:
    """'3d', '2h', '30m' gibi süreleri saniyeye çevirir."""
    ...
    if not muaf_mi(message.author, s.get("muaf_roller", [])):
        if s.get("kufur_filtre", True) and kufur_var_mi(message.content, s.get("kara_liste", [])):
            try:
                ...
                txt = f"⚠️ {message.author.mention}, uygunsuz kelime kullandın! Mesajın silindi."
                if sure > 0:
                    txt += f" {sure} saniye beklemelisin."
                uyari = await message.channel.send(txt)
        ...
        if s.get("reklam_filtre", True) and reklam_var_mi(message.content, s.get("reklam_liste", [])):
            try:
                ...
                txt = f"📢 {message.author.mention}, reklam/link paylaşmak yasaktır! Mesajın silindi."
                if sure > 0:
                    txt += f" {sure} saniye beklemelisin."
                uyari = await message.channel.send(txt)
                await uyari.delete(delay=5)
...
async def hosgeldin_karti_olustur(member: discord.Member, hosgeldin: bool = True) -> discord.File:
    """Hoşgeldin/Hoşçakal kartı oluşturur."""
    W, H = 800, 250
    ...
    baslik = "HOŞ GELDİN!" if hosgeldin else "HOŞÇA KAL!"
    draw.text((220, 55), baslik, fill=renk, font=font_buyuk)
    draw.text((220, 105), member.name, fill=(255, 255, 255), font=font_kucuk)
    alt_metin = f"Seninle {member.guild.member_count} kişi olduk!" if hosgeldin else "Bize veda etmen üzücü oldu 💔"
    draw.text((220, 145), alt_metin, fill=(180, 180, 180), font=font_mini)
...
async def owner(ctx):
    embed = discord.Embed(
        title="👑 Sunucu Kurucusu",
        description="**FelxeTheGreat**",
        color=0xFFD700
    )
...
async def ban(ctx, uye: discord.Member, sure_str: str = None, *, sebep: str = "Belirtilmedi"):
    ...
    embed = discord.Embed(
        ...
        description=f"**{uye}** sunucudan uzaklaştırıldı.",
        color=0xFF4444
    )
    ...
    await ctx.send(embed=embed)
    await log_gonder(ctx.guild, get_guild_settings(ctx.guild.id), f"🔨 **Ban** | {uye} | Süre: {sure_metin} | Sebep: {sebep} | Yetkili: {ctx.author}")
...
async def mute(ctx, uye: discord.Member, sure_str: str = None, *, sebep: str = "Belirtilmedi"):
    ...
    embed.add_field(name="Yetkili", value=ctx.author.mention, inline=True)
    await ctx.send(embed=embed)
    await log_gonder(ctx.guild, get_guild_settings(ctx.guild.id), f"🔇 **Mute** | {uye} | Süre: {sure_metin} | Sebep: {sebep} | Yetkili: {ctx.author}")

@sil.error
@ban.error
@mute.error
async def komut_hatasi(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send(f"❌ Eksik parametre! Kullanım:\n`x!ban @kişi 3d sebep`\n`x!mute @kişi 3h sebep`\n`x!sil 10`", delete_after=8)
...
@app_commands.describe(kanal="Hoşgeldin mesajının gönderileceği kanal")
@app_commands.checks.has_permissions(administrator=True)
...
@app_commands.describe(kelime="Yasaklanacak kelime veya link")
@app_commands.checks.has_permissions(administrator=True)
...
@app_commands.describe(kelime="Silinecek kelime veya link")
@app_commands.checks.has_permissions(administrator=True)
...
async def otomatikmesaj_ayarla(interaction: discord.Interaction, anahtar: str, cevap: str):
    ...
    save_guild_settings(interaction.guild.id, s)
    await interaction.response.send_message(f"✅ Otomatik mesaj eklendi! **{anahtar}** → {cevap}", ephemeral=True)
